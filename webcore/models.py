...
class AuthorDraftManager(models.Manager):
	def active(self, *args, **kwargs):
		return super(AuthorDraftManager, self).filter(active=True).filter(draft=False).filter(published_on__lte=timezone.now())
...
class ABC_AuthorDraft(ABC_TitleSlug):
	...
	def clean(self):
		...
		if not self.draft and not self.published_on:
